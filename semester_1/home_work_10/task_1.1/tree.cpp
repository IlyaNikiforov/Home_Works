...
void addCode(TreeNode *parent, TreeNode *&node, char symbol)
{
	for (int i = 0; i < parent->index; i++)
		node->code[i] = parent->code[i];
	node->code[parent->index] = symbol;
...
void getCoded(TreeNode *&node)
{
	...
	if (node->left != nullptr)
	{
		addCode(node, node->left, '0');
		getCoded(node->left);
	}

	if (node->right != nullptr)
	{
		addCode(node, node->right, '1');
		getCoded(node->right);
	}
}
...
void findCode(TreeNode *node, char a, ofstream &out)
{
	if (node->symbol == a)
	{
		for (int i = 0; i < node->index; i++)
	...
	if (node->left != nullptr)
		findCode(node->left, a, out);

	if (node->right != nullptr)
		findCode(node->right, a, out);
}
